...
const experienceMeta={
  ...
  eye:{title:"Eye Tracking V2",desc:"Rastreamento ocular com calibração multiponto personalizada.",help:[["9️⃣ Calibração","Olhe para cada ponto roxo até o círculo completar. São 9 posições."],["👁️ Cursor","Depois da calibração, o ponto verde usa uma transformação calculada para teus olhos."],["🎯 Teste","Olhe para o alvo roxo por um instante para marcar acerto."],["📊 Qualidade","A tela mostra uma estimativa da qualidade da calibração."]]},
  games:{title:"Gesture Games",desc:"Arcade controlado por mãos, movimento e agora também olhar calibrado.",help:[["🥊 Boxe","Acerte os alvos com movimentos rápidos das duas mãos."],["🧤 Goleiro","Use as palmas como luvas para defender os chutes."],["🏀 Basquete","Faça pinça na bola, mova e solte para arremessar."],["👁️ Eye Aim","Calibre os olhos em 9 pontos e mire apenas olhando."]]}
};
...
async function openExperience(name){
  ...
  if(name==="eye"){
    addAction("🎯 Calibrar 9 pontos",()=>{eyeTracking.startCalibration();showToast("Siga os pontos com os olhos 👁️")},true);
    addAction("Novo alvo",()=>eyeTracking.randomTarget());
    addAction("Recalibrar",()=>eyeTracking.startCalibration());
  }
  if(name==="games"){
    gestureGames.reset(overlay.width||960,overlay.height||540);gameHandSamples=[];
    const choose=(label,key,accent=false)=>addAction(label,async()=>{
      if(key==="eyeaim"){
        try{await vision.ensureFace()}catch(e){console.error(e);showToast("Falha ao carregar Face Tracking");return}
        if(!eyeTracking.calibrated){eyeTracking.startCalibration();showToast("Primeiro: calibração ocular de 9 pontos 👁️")}
      }
      gestureGames.setGame(key,overlay.width||960,overlay.height||540);modeEl.textContent=`Games • ${gestureGames.title()}`;showToast(`${gestureGames.title()} iniciado 🎮`);
    },accent);
    choose("🏓 Pong","pong",true);choose("🍉 Fruit Slice","fruit");choose("🧱 Brick Breaker","brick");choose("✊✋✌️ RPS","rps");
    choose("🥊 Boxe","boxing");choose("🧤 Goleiro","goalie");choose("🏀 Basquete","basket");choose("👁️ Eye Aim","eyeaim");
    addAction("🎯 Calibrar olhos",async()=>{try{await vision.ensureFace();eyeTracking.startCalibration();showToast("Siga os 9 pontos 👁️")}catch(e){console.error(e)}});
    addAction("Reiniciar jogo",()=>gestureGames.reset(overlay.width||960,overlay.height||540));
    modeEl.textContent=`Games • ${gestureGames.title()}`;
  }
}

function closeExperience(){currentExperience=null;experienceView.classList.remove("active");hubView.classList.add("active");airDraw.resetStroke();motionEngine.reset();gameHandSamples=[]}
...
function buildGameHands(now){
  const out=lastLandmarks.slice(0,2).map((lm,i)=>{
    const p=n=>point(lm,n),index=p(8),thumb=p(4),pinch=avg(index,thumb),palm=avg(p(0),p(5),p(9),p(13),p(17)),gesture=gestureEngine.classify(lm,overlay.width,overlay.height),prev=gameHandSamples[i];
    let vx=0,vy=0,speed=0;if(prev){const dt=Math.max((now-prev.t)/1000,.001);vx=(palm.x-prev.x)/dt;vy=(palm.y-prev.y)/dt;speed=Math.hypot(vx,vy)}gameHandSamples[i]={x:palm.x,y:palm.y,t:now};
    return{gesture,palm,index,pinch,pinching:gesture==="pinch",vx,vy,speed};
  });
  gameHandSamples.length=out.length;return out;
}

function updateAbsoluteCinemaCombo(now){
  if(lastLandmarks.length<2){comboSince=0;comboLatched=false;return}
  const gestures=lastLandmarks.slice(0,2).map(lm=>gestureEngine.classify(lm,overlay.width,overlay.height)),matched=gestures.includes("peace")&&gestures.includes("open");
  if(!matched){comboSince=0;comboLatched=false;return}if(!comboSince)comboSince=now;if(!comboLatched&&now-comboSince>=450){comboLatched=true;if(absoluteCinema.trigger(now))showToast("ABSOLUTE CINEMA 🎬")}
}
...
function render(now){
  ...
  if(currentExperience==="games"){
    gestureGames.render(oc,overlay.width,overlay.height,now);
    if(gestureGames.game==="eyeaim"&&eyeTracking.calibrated)eyeTracking.render(oc,overlay.width,overlay.height,now,{practice:false,cursor:true});
    if(gestureGames.game==="eyeaim"&&eyeTracking.isCalibrating)eyeTracking.render(oc,overlay.width,overlay.height,now,{practice:false,cursor:false});
  }
  for(const lm of lastLandmarks)drawSkeleton(lm);
  if(hand.point&&currentExperience!=="eye"&&currentExperience!=="games"){oc.save();oc.beginPath();oc.arc(hand.point.x,hand.point.y,Math.max(10,overlay.width/95),0,Math.PI*2);oc.fillStyle=hand.gesture==="point"?"#72ffd5":"#fff";oc.shadowBlur=18;oc.shadowColor=oc.fillStyle;oc.fill();oc.restore()}
...
function updateDebug(){
  ...
  if(currentExperience==="eye"){hitsEl.textContent=String(eyeTracking.hits);modeEl.textContent=eyeTracking.isCalibrating?`Calibrando ${eyeTracking.calibration.index+1}/9`:eyeTracking.calibrated?`Eye V2 • ${Math.round(eyeTracking.quality*100)}% qualidade`:"Eye Tracking • não calibrado"}
  if(currentExperience==="face"){const m=faceLab.metrics;modeEl.textContent=`🙂 ${Math.round(m.smile*100)}% • 😮 ${Math.round(m.mouth*100)}% • 👁 ${Math.round(Math.max(m.blinkL,m.blinkR)*100)}%`}
  if(currentExperience==="games"){hitsEl.textContent=gestureGames.hud();modeEl.textContent=`Games • ${gestureGames.title()}`}
}
...
function loop(now=performance.now()){
  ...
  if(result){
    ...
    if(currentExperience==="eye")eyeTracking.update(lastFaceResult,now,{practice:true});
    if(currentExperience==="games"&&gestureGames.game==="eyeaim")eyeTracking.update(lastFaceResult,now,{practice:false});
    tickFps();
  }
  ...
  if(currentExperience==="games"){
    const hands=buildGameHands(now),eye=gestureGames.game==="eyeaim"?eyeTracking.cursor:null;
    gestureGames.update({hand,hands,eye},dt,now,overlay.width,overlay.height);
  }
