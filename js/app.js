...
const experienceMeta={
  ...
  eye:{title:"Eye Tracking",desc:"Rastreamento ocular experimental usando as íris do Face Landmarker.",help:[["🎯 Calibrar","Olhe para o centro da câmera e toque em Calibrar centro."],["👁️ Cursor","O ponto verde representa uma estimativa da direção do olhar."],["⏱️ Dwell","Olhe para o alvo roxo por um instante para selecioná-lo."],["🧪 Beta","A calibração ainda será melhorada antes do Eye Aim."]]},
  games:{title:"Gesture Games",desc:"Arcade controlado pela câmera usando mãos, gestos e movimento.",help:[["🏓 Pong","Mova a palma para cima e para baixo e controle a raquete esquerda."],["🍉 Fruit Slice","Levante o indicador e corte as frutas com movimentos rápidos."],["🧱 Brick Breaker","Mova a mão horizontalmente para controlar a plataforma."],["✊✋✌️ RPS","Segure pedra, papel ou tesoura por um instante para jogar contra a CPU."]]}
};
...
async function openExperience(name){
  ...
  hubView.classList.remove("active");experienceView.classList.add("active");
  experienceName.textContent=meta.title;experienceDesc.textContent=meta.desc;modeEl.textContent=meta.title;
  paint.style.opacity=name==="draw"?"1":".12";
  hitBox.style.display=(name==="emoji"||name==="eye"||name==="games")?"block":"none";
  hitsEl.textContent="0";airDraw.resetStroke();motionEngine.reset();moduleActions.innerHTML="";
  ...
  if(name==="games"){
    gestureGames.reset(overlay.width||960,overlay.height||540);
    const choose=(label,key)=>addAction(label,()=>{gestureGames.setGame(key,overlay.width||960,overlay.height||540);modeEl.textContent=`Games • ${gestureGames.title()}`;showToast(`${gestureGames.title()} iniciado 🎮`)},key==="pong");
    choose("🏓 Pong","pong");
    choose("🍉 Fruit Slice","fruit");
    choose("🧱 Brick Breaker","brick");
    choose("✊✋✌️ RPS","rps");
...
function updateHand(landmarks,now){
  ...
  if(lastHandSample){const dt=Math.max((now-lastHandSample.t)/1000,.001);vx=(palm.x-lastHandSample.x)/dt;vy=(palm.y-lastHandSample.y)/dt;speed=Math.hypot(vx,vy)}
...
function render(now){
  ...
  if(currentExperience==="games")gestureGames.render(oc,overlay.width,overlay.height,now);
  for(const lm of lastLandmarks)drawSkeleton(lm);
  if(hand.point&&currentExperience!=="eye"&&currentExperience!=="games"){
    oc.save();oc.beginPath();oc.arc(hand.point.x,hand.point.y,Math.max(10,overlay.width/95),0,Math.PI*2);oc.fillStyle=hand.gesture==="point"?"#72ffd5":"#fff";oc.shadowBlur=18;oc.shadowColor=oc.fillStyle;oc.fill();oc.restore();
  }
...
function loop(now=performance.now()){
  if(!running)return;
  resize();
  const dt=Math.min((now-lastFrameTime)/1000,.04);lastFrameTime=now;
  ...
  if(result){
    ...
    if(currentExperience==="spell"){
      ...
      if(motion){spellCasting.trigger(motion,now);const pct=Math.round((motion.confidence??1)*100),names={circle:`Portal detectado ⭕ • ${pct}%`,lightning:`Raio detectado ⚡ • ${pct}%`,slash:`Corte detectado 🗡️ • ${pct}%`};showToast(names[motion.type]||motion.type)}
    }
  ...
  if(currentExperience==="games")gestureGames.update(hand,dt,now,overlay.width,overlay.height);
